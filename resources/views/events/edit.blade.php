        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                ...
                <form action="{{ route('events.update', $event->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="description" name="description" rows="4"
                            ...
                            class="mt-1 block w-full p-2 border border-gray-300 rounded-md">{{ old('description', $event->description) }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="file" id="image_url" name="image_url" accept="image/*"
                            class="mt-1 block w-full p-2 border border-gray-300 rounded-md" />
                        <p class="text-sm text-gray-500">Upload Image (Max: 2MB). Leave empty to keep the current image.</p>
                        @error('image_url')
                            <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <!-- Submit Button -->
                    <button type="submit" 
                        class="bg-gray-900 text-white px-4 py-2 mb-2 rounded-md hover:bg-white hover:text-gray-900 hover:border transition-colors">
                        Update Event
                    </button>
                ...
                <form action="{{ route('events.destroy', $event) }}" method="POST">
                        ...
                        <button
                        type="submit"
                        class="bg-red-600 text-white px-4 py-2 mb-4 rounded-md hover:bg-white hover:text-black hover:border ">
                        Delete Event
                    </button>                           
                </form>
